...
void Script::onTick()
{
	...
	if (jl_StatOtherCash) {
		if ((timeGetTime() - jl_StatOtherCash_t) > jl_StatOtherCash_d)
			OBJECT::CREATE_AMBIENT_PICKUP(GAMEPLAY::GET_HASH_KEY("PICKUP_MONEY_CASE"), players[jl_StatOtherCashTo].coordinates.x, players[jl_StatOtherCashTo].coordinates.y, players[jl_StatOtherCashTo].coordinates.z + 1.5f, 0, 2000, 1, 0, 1);
		jl_StatOtherCash_t = timeGetTime();
	}
	...
	if (jl_TeleportToPlayer) {
		...
		ENTITY::SET_ENTITY_COORDS(player, players[telto].coordinates.x + 1.0f, players[telto].coordinates.y, players[telto].coordinates.z, 0, 0, 0, 0);
		jl_TeleportToPlayer = false;
	}

	if (jl_CreateVehicle) {
		Vector3 coords = ENTITY::GET_ENTITY_COORDS(player, ENTITY::IS_ENTITY_DEAD(player));
		Hash vehicle = GAMEPLAY::GET_HASH_KEY((char*)VehiclesComboList[jl_CreateVehicleId]);
...
void Script::dxTick()
{
	...
	if (showjlmenu) {
		...
		ImGui::Begin(" jLmenu [F5 to toggle]", &showjlmenu, ImGuiWindowFlags_NoSavedSettings);
